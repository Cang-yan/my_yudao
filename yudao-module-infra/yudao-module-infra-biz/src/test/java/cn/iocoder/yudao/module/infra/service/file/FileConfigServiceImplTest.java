...
public class FileConfigServiceImplTest extends BaseDbUnitTest {
    ...
    public void testInitLocalCache() {
        // mock 数据
        FileConfigDO configDO1 = randomFileConfigDO().setId("1").setMaster(true);
        fileConfigMapper.insert(configDO1);
        FileConfigDO configDO2 = randomFileConfigDO().setId("2").setMaster(false);
        fileConfigMapper.insert(configDO2);
        // mock fileClientFactory 获得 master
        FileClient masterFileClient = mock(FileClient.class);
        when(fileClientFactory.getFileClient(eq("1"))).thenReturn(masterFileClient);

        // 调用
        fileConfigService.initFileClients();
        // 断言 fileClientFactory 调用
        verify(fileClientFactory).createOrUpdateFileClient(eq("1"),
                eq(configDO1.getStorage()), eq(configDO1.getConfig()));
        verify(fileClientFactory).createOrUpdateFileClient(eq("2"),
    ...
    public void testCreateFileConfig_success() {
        ...
        // 调用
        String fileConfigId = fileConfigService.createFileConfig(reqVO);
        // 断言
    ...
    public void testUpdateFileConfigMaster_notExists() {
        // 调用, 并断言异常
        assertServiceException(() -> fileConfigService.updateFileConfigMaster(randomLongId().toString()), FILE_CONFIG_NOT_EXISTS);
    }
