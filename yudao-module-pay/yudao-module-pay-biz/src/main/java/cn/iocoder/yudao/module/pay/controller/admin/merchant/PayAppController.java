...
public class PayAppController {
    ...
    public CommonResult<String> createApp(@Valid @RequestBody PayAppCreateReqVO createReqVO) {
    ...
    public CommonResult<Boolean> deleteApp(@RequestParam("id") String id) {
    ...
    public CommonResult<PayAppRespVO> getApp(@RequestParam("id") String id) {
    ...
    @PreAuthorize("@ss.hasPermission('pay:app:query')")
    public CommonResult<List<PayAppRespVO>> getAppList(@RequestParam("ids") Collection<String> ids) {
    ...
    public CommonResult<PageResult<PayAppPageItemRespVO>> getAppPage(@Valid PayAppPageReqVO pageVO) {
        ...
        // 得到所有的应用编号，查出所有的渠道
        Collection<String> payAppIds = CollectionUtils.convertList(pageResult.getList(), PayAppDO::getId);
        ...
        // 得到所有的商户信息
        Collection<String> merchantIds = CollectionUtils.convertList(pageResult.getList(), PayAppDO::getMerchantId);
        Map<String, PayMerchantDO> deptMap = merchantService.getMerchantMap(merchantIds);
    ...
    @ApiImplicitParam(name = "merchantId", value = "商户ID", required = true, example = "1", dataTypeClass = String.class)
