...
public interface PayOrderMapper extends BaseMapperX<PayOrderDO> {
    ...
    default List<PayOrderDO> findByIdListQueryOrderSubject(Collection<String> idList) {
     ...
     * @return 条数
     */
    default Long selectCount(String appId, Integer status) {
        return selectCount(new LambdaQueryWrapper<PayOrderDO>()
                .eq(PayOrderDO::getAppId, appId)
                .in(PayOrderDO::getStatus, status));
    }

    default PayOrderDO selectByAppIdAndMerchantOrderId(String appId, String merchantOrderId) {
        return selectOne(new QueryWrapper<PayOrderDO>().eq("app_id", appId)
                .eq("merchant_order_id", merchantOrderId));
    }

    default int updateByIdAndStatus(String id, Integer status, PayOrderDO update) {
