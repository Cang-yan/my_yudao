...
public class RoleController {
    ...
    @ApiOperation("删除角色")
    @ApiImplicitParam(name = "id", value = "角色编号", required = true, example = "1024", dataTypeClass = String.class)
    ...
    public CommonResult<RoleRespVO> getRole(@RequestParam("id") String id) {
    ...
    public CommonResult<List<RoleSimpleRespVO>> getSimpleRoles() {
        ...
        if (!userId.equals("1")&&permissionApi.hasAnyRoles(userId, "admin_custom")) {
            List<RoleDO> roleDOList = new ArrayList<>();
            for (RoleDO roleDO : list) {
                if (roleDO.getCode().equals("super_admin") || roleDO.getCode().equals("common")||roleDO.getCode().equals("admin_custom")) continue;
                roleDOList.add(roleDO);
            }
            return success(RoleConvert.INSTANCE.convertList02(roleDOList));
