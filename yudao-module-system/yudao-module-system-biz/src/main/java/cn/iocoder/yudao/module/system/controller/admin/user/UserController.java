...
public class UserController {
    ...
    public CommonResult<PageResult<UserPageItemRespVO>> getUserPage(@Valid UserPageReqVO reqVO) {
        ...
        // 获得拼接需要的数据
        Collection<String> deptIds = convertList(pageResult.getList(), AdminUserDO::getDeptId);
        Map<String, DeptDO> deptMap = deptService.getDeptMap(deptIds);
        // 拼接结果返回
      /*  List<UserPageItemRespVO> userList = new ArrayList<>(pageResult.getList().size());
        ...
        List<UserPageItemRespVO> userList = new ArrayList<>();
        pageResult.getList().forEach(user -> {
            if (!user.getId().equals("1")&& !user.getId().equals(userId)) {
            UserPageItemRespVO respVO = UserConvert.INSTANCE.convert(user);
            respVO.setDept(UserConvert.INSTANCE.convert(deptMap.get(user.getDeptId())));
            userList.add(respVO);
            }
    ...
    public void exportUsers(@Validated UserExportReqVO reqVO,
        ...
        // 获得拼接需要的数据
        Collection<String> deptIds = convertList(users, AdminUserDO::getDeptId);
        Map<String, DeptDO> deptMap = deptService.getDeptMap(deptIds);
    ...
    public void importTemplate(HttpServletResponse response) throws IOException {
        ...
        List<UserImportExcelVO> list = Arrays.asList(
                UserImportExcelVO.builder().username("yunai").deptId("1").email("yunai@iocoder.cn").mobile("15601691300")
                        .nickname("芋道").status(CommonStatusEnum.ENABLE.getStatus()).sex(SexEnum.MALE.getSex()).build(),
                UserImportExcelVO.builder().username("yuanma").deptId("2").email("yuanma@iocoder.cn").mobile("15601701300")
