...
public class DeptServiceImpl implements DeptService {
    ...
    private volatile Map<String, DeptDO> deptCache;
    /**
     * 父部门缓存
     ...
     * 这里声明 volatile 修饰的原因是，每次刷新时，直接修改指向
     */
    private volatile Multimap<String, DeptDO> parentDeptCache;
    /**
    ...
    public synchronized void initLocalCache() {
        ...
        // 构建缓存
        ImmutableMap.Builder<String, DeptDO> builder = ImmutableMap.builder();
        ImmutableMultimap.Builder<String, DeptDO> parentBuilder = ImmutableMultimap.builder();
    ...
    @Override
    public List<DeptDO> getDeptsByParentIdFromCache(String parentId, boolean recursive) {
        if (parentId == null) {
     ...
     */
    private void getDeptsByParentIdFromCache(List<DeptDO> result, String parentId, int recursiveCount,
                                             Multimap<String, DeptDO> parentDeptMap) {
        // 递归次数为 0，结束！
        if (recursiveCount == 0) {
            return;
        }
    ...
    @Override
    public List<DeptDO> getDepts(Collection<String> ids) {
    ...
    public void validDepts(Collection<String> ids) {
        ...
        Map<String, DeptDO> deptMap = CollectionUtils.convertMap(depts, DeptDO::getId);
        // 校验
        ids.forEach(id -> {
    ...
    @Override
    public List<DeptDO> getSimpleDepts(Collection<String> ids) {
        return deptMapper.selectBatchIds(ids);
    }
