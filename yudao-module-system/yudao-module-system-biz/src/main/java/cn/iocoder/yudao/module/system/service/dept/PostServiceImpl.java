...
public class PostServiceImpl implements PostService {
    ...
    @Override
    public List<PostDO> getPosts(Collection<String> ids, Collection<Integer> statuses) {
    ...
    @Override
    public PostDO getPost(String id) {
        return postMapper.selectById(id);
    }

    private void checkCreateOrUpdate(String id, String name, String code) {
        // 校验自己存在
        checkPostExists(id);
        // 校验岗位名的唯一性
    ...
    private void checkPostNameUnique(String id, String name) {
    ...
    private void checkPostCodeUnique(String id, String code) {
    ...
    @Override
    public void validPosts(Collection<String> ids) {
        if (CollUtil.isEmpty(ids)) {
            return;
        }
        // 获得岗位信息
        List<PostDO> posts = postMapper.selectBatchIds(ids);
        Map<String, PostDO> postMap = convertMap(posts, PostDO::getId);
        // 校验
        ids.forEach(id -> {
